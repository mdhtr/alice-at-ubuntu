...
#########
### Convert
# - color to color OR black-and-white to black-and-white
# - color or grayscale to black-and-white (with mkbitmap conversion)
...
#	name="${file%.*}"
	...
	if [ "${file##*.}" != "pnm" ] && [ "${file##*.}" != "pbm" ]; then
		...
		if [ $colorspace = "Grayscale" ]; then
			convert "$file" -resize 50% "${file%.*}.pnm"
			mkbitmap "${file%.*}.pnm"
			rm "${file%.*}.pnm"
			file="${file%.*}.pbm"
		elif [ $colorspace = "Bilevel" ]; then
			convert "$file" -resize 50% "${file%.*}.pbm"
			file="${file%.*}.pbm"
		else # keep color
			convert "$file" -resize 50% "${file%.*}.pnm"
			file="${file%.*}.pnm"
		fi
	elif [ "${file##*.}" = "pnm" ] || [ "${file##*.}" = "pbm" ]; then
		convert "$file" -resize 50% "${file%.*}_r.${file##*.}"
		file="${file%.*}_r.${file##*.}"
	fi
	converted+=("$file")
